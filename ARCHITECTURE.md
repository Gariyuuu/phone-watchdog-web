...
## Relationship to `~/Projects/phone-watchdog` [Verified 2026-08-17]

**No shared backend, no runtime coupling.** `~/Projects/phone-watchdog`
(no trailing `-web`) is a separate git repository containing a small
local Python script (`monitor.py`, YOLO-based desktop phone detector) —
this repo's client-side TensorFlow.js detector is its independently-built
spiritual successor for the same personal idea, not a frontend for it.
Confirmed directly for this documentation pass: `monitor.py` contains no
`requests`/HTTP calls of any kind and no reference to
`phone-watchdog-web` or its Vercel URL; this repo's own code
(`src/app/page.tsx`, `src/app/api/catches/route.ts`) contains no
reference to the Python repo, no shared package, and no shared database
— this repo's Postgres `catches` table (Neon, via `@vercel/postgres`) has
no counterpart in `monitor.py` at all: it imports no database library
(`sqlite3`, `psycopg`, etc.) and performs no persistence of any kind —
detections are ephemeral, shown only in its `tkinter` window.
Do not assume changes to one repo affect the other, and do not route one
through the other without that being the explicit, deliberate point of a
future task.
...
- **Entry point:** `src/app/layout.tsx` (Server Component) — wraps
  ...
  `next/font/google`, sets page the title tag/`description` metadata. No
...
1. User clicks **Start**. `start()` (`src/app/page.tsx`) calls
...
5. `triggerAlarm()` sets `caught = true`, starts the Web Audio siren
   ...
   passes through `src/proxy.ts` (Basic Auth re-checked on every request —
   there's no session, so this happens on every single fetch), reaches
   `src/app/api/catches/route.ts`'s `POST` handler, which calls `ensureTable()` then inserts a
...
single shared-secret gate. `src/proxy.ts` runs on every matched request
...
check anywhere in `src/app/api/catches/route.ts` — any authenticated caller can read, create,
or clear *any* catch row, not just "their own" (there's no concept of
"whose" a catch is).

## Database access flow

`src/app/api/catches/route.ts` imports `sql` from `@vercel/postgres`, which lazily opens a
...
- **Server:** every `src/app/api/catches/route.ts` handler wraps its body in `try/catch` and
...
- **`src/proxy.ts`:** returns a plain-text `401` with a `WWW-Authenticate`
...
The only boundary in this app is `src/proxy.ts`'s shared-password check.
